 ...
 */
angular.module('hw8',['angularUtils.directives.dirPagination']);

angular.module('hw8').controller('hw8Controller', ['$scope','$http', function($scope, $http){

    $scope.initHW8 = function(){
    ...
    $scope.displayLegislatorsByStateInfo = function() {
        $http({method: "GET", url: "http://localhost:70/hw8/hw8.php", params: {"type":"legislators"}})
    ...
    $scope.showLegislatorDetails = function(legislator) {
        ...
        $http({
            method: "GET",
            url: "http://localhost:70/hw8/hw8.php",
   ...
   /* $scope.displayBillsContent = function() {
      $http({
        ...
        $scope.bills = JSON.parse(resp.data);
          $scope.bills = $scope.bills.results;
          $scope.newBills = [];
          for(var i = 0; i < $scope.bills.length; i++) {
              if(!$scope.bills[i].history.active){
                  $scope.newBills.push($scope.bills[i]);
              }
          }
      }, function failure(err){
            console.log(err);
    ...
    $scope.displayCommitteesHouseInfo = function() {
        ...
        $http({method: "GET", url: "http://localhost:70/hw8/hw8.php", params: {"type":"committees"}})
            .then(function success(resp){
